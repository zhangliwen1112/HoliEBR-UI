...
class Test_stolocation_move:
    ...
    # 移动库位
    @pytest.mark.parametrize('target,ID',[('1234','LOT0000003 0008'),('3333','LOT0000003 0008')])
    def test_stolocation_move(self,target,ID):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        stolocation_move(target,ID)
        sleep(2)
        assert is_text_present('保存成功')
    ...
    def test_stolocation_detail(self):
        ...
        sleep(2)
        assert new_js_text('//form/div/div[2]/div/div/div[1]/div/input') == 'LOT0000003 0007'
