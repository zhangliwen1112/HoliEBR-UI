# -*- coding: utf-8 -*- 
# @Time : 2021/5/20 17:03 
# @Author : 张丽雯 
# @File : test_reserviorlabel_abnormal.py
# @中文描述 :  标签-库区，异常场景测试用例
import pytest
import sys
...
class Test_reserviorlabel_Abnormal:
    ...
    def test_reserviorlabel_code_abnormal_001(self, code, name, desc, zpl):
    ...
    def test_reserviorlabel_code_abnormal_002(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_reserviorlabel_code_abnormal_003(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_reserviorlabel_name_abnormal_001(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_reserviorlabel_name_abnormal_002(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_reserviorlabel_zpl_abnormal_001(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
