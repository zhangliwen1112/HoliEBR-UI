# -*- coding: utf-8 -*- 
# @Time : 2021/5/20 17:18 
# @Author : 张丽雯 
# @File : test_weighlabel_abnormal.py 
# @中文描述 :  标签-称量标签，异常测试用例
import pytest
import sys
...
class Test_weighlabel_Abnormal:
    def test_weighlabel_login(self):
        login_label()
        new_click(weigh)
        sleep(2)
    ...
    def test_weighlabel_code_abnormal_001(self, code, name, desc, zpl):
    ...
    def test_weighlabel_code_abnormal_002(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_weighlabel_code_abnormal_003(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_weighlabel_name_abnormal_001(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_weighlabel_name_abnormal_002(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_weighlabel_zpl_abnormal_001(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
