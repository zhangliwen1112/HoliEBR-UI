# -*- coding: utf-8 -*- 
# @Time : 2021/5/20 16:28 
# @Author : 张丽雯 
# @File : test_opalletlabel_abnormal.py 
# @中文描述 :  标签-产出托盘，异常场景测试用例
import pytest
import sys
...
class Test_opalletlabel_Abnormal:
    def test_opalletlabel_login(self):
        login_label()
        new_click(opallet)
        sleep(2)
    ...
    def test_opalletlabel_code_abnormal_001(self, code, name, desc, zpl):
    ...
    def test_opalletlabel_code_abnormal_002(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_opalletlabel_code_abnormal_003(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_opalletlabel_name_abnormal_001(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_opalletlabel_name_abnormal_002(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_opalletlabel_zpl_abnormal_001(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
