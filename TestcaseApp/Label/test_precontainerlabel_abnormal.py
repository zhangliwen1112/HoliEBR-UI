# -*- coding: utf-8 -*- 
# @Time : 2021/5/20 16:56 
# @Author : 张丽雯 
# @File : test_precontainerlabel_abnormal.py 
# @中文描述 :  标签-预产出容器，异常测试用例
import pytest
import sys
...
class Test_precontainerlabel_Abnormal:
    ...
    def test_precontainerlabel_code_abnormal_001(self, code, name, desc, zpl):
    ...
    def test_precontainerlabel_code_abnormal_002(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_precontainerlabel_code_abnormal_003(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_precontainerlabel_name_abnormal_001(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_precontainerlabel_name_abnormal_002(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
    ...
    def test_precontainerlabel_zpl_abnormal_001(self, code, name, desc, zpl):
        log.info("开始执行用例%s" % sys._getframe().f_code.co_name)
        label_add(code, name, desc, zpl)
