...
# 新增处方页面
def formula_add_web(formula_name, reference_num, reference_unit, equivalent_weight,weight_unit):
    new_click(add)
    sleep(1)
    ...
    sleep(1)
    if new_get_text("//tbody//i") == 'check_box':
        new_click(product_submit)
    else:
    ...
    sleep(1)
    # 异常提示
    if new_page_source('名称由字母、数字、汉字、/_-组成，不能以特殊字符开头'):
        new_get_text(error_message)
    elif new_page_source('请输入1到100个字符' or '请输入' or '请输入精度为3位的非0小数' or '请输入小于9999999999的数值'):
...
def formula_add_material_001(maternum,unit):
    ...
    driver.find_elements_by_xpath(material_submit)[1].click()
